        ...
        <main className="Component-main">
          <section className="Schedule-section" id="General-Assembly">
            {displayedData.map((item, index) => (
              ...
              >
              {displayOrder
                .filter((key) => item[key] !== null && item[key] !== "" && item[key] !== undefined) // Exclude null, empty, or undefined values
                .map((key) => (
                  <p key={key} style={{ margin: "0.5rem 0" }}>
                    <strong>{columnNames[key]}:</strong>{" "}
                    {key === "StartDate" ? formatDate(item[key] as string) : item[key]}
                  </p>
                ))}
